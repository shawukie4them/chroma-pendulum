;(function(){
  ...
  gsap.set(GLASSES[1], {'--light-alpha': CONFIG.GLASS.ALPHA.ON, 
  ...
  gsap.set(FILAMENTS[1], {'--filament-lightness': CONFIG.FILAMENT.LIGHTNESS.ON})
  const SWING_LEFT_TL = new timeline()
  .add(to(CHORDS[4], {
    ease: `power${SPEEDS.EASE}.out`,
    duration: SPEEDS.SWING,
    morphSVG: {d: CURVES.LEFT}
  }), 0)
  .add(to(BULBS[4], {
    ease: `power${SPEEDS.EASE}.out`,
  ...
  .add(to(CHORDS[1], SPEEDS.SWING, {
    ease: `power${SPEEDS.EASE}.in`,
    morphSVG: {d: STRAIGHTS.RIGHT},
  }), 0)
  .add(to(BULBS[1], SPEEDS.SWING, {
    ease: `power${SPEEDS.EASE}.in`,
    rotate: 0
  }), 0)
  .add(to(GLASSES[1], {
  ...
  .add(to([GLASSES[2], GLASSES[5], GLASSES[0], GLASSES[3]], {
    duration: SPEEDS.ON,
  ...
  .add(to([BLOOMS[2], BLOOMS[5], BLOOMS[0], BLOOMS[3]], {
    duration: SPEEDS.ON * 5,
    stagger: SPEEDS.STAGGER,
    scale: 1.25,
    opacity: 0
  }), 'SWITCHING_ON')
  .add(to([FILAMENTS[2], FILAMENTS[5], FILAMENTS[0], FILAMENTS[3]], {
    duration: SPEEDS.ON,
    stagger: SPEEDS.STAGGER,
    '--filament-lightness': CONFIG.FILAMENT.LIGHTNESS.ON
  }), 'SWITCHING_ON')
  ...
  .add(to([GLASSES[3], GLASSES[0], GLASSES[5], GLASSES[2]], {
    duration: SPEEDS.ON,
    stagger: SPEEDS.STAGGER,
    '--light-alpha': CONFIG.GLASS.ALPHA.ON,
    '--glass-saturation': CONFIG.GLASS.SATURATION.ON,
  ...
  .add(fromTo([BLOOMS[3], BLOOMS[0], BLOOMS[5], BLOOMS[2]], {
    scale: 0,
    opacity: 1
  },
  {
  ...
  .add(to([FILAMENTS[3], FILAMENTS[0], FILAMENTS[5], FILAMENTS[2]], {
    duration: SPEEDS.ON,
    stagger: SPEEDS.STAGGER,
    '--filament-lightness': CONFIG.FILAMENT.LIGHTNESS.ON
  }), 'SWITCHING_ON_2')
